...
public class ClientController {
	...
	public @ResponseBody UserStatus addClient(
		...
		try {
			...
			NotificationDTO notificationDTO = notificationService.getNotificationByCode((messageSource.getMessage(DreamConstructionConstants.CLIENT_ADD_NOTIFICATION, null, null)));
			emailNotificationClient(clientDTO, request, response, notificationDTO);
			return new UserStatus(1, messageSource.getMessage(DreamConstructionConstants.CLIENT_ADDED, null, null));
	...
	public @ResponseBody UserStatus updateClient(@RequestBody ClientDTO clientDTO,
		...
		try {
	    	...
	    	NotificationDTO notificationDTO = notificationService.getNotificationByCode((messageSource.getMessage(DreamConstructionConstants.CLIENT_UPDATE_NOTIFICATION, null, null)));
	    	emailNotificationClient(clientDTO, request, response, notificationDTO);
			return new UserStatus(1, messageSource.getMessage(DreamConstructionConstants.CLIENT_UPDATE, null, null));
		} catch (Exception e) {
			logger.error(e);
			e.printStackTrace();
	...
	public @ResponseBody UserStatus deleteClient(@PathVariable("id") long id) {
		...
		try {
			...
			return new UserStatus(1, messageSource.getMessage(
					DreamConstructionConstants.CLIENT_DELETE, null, null));
		} catch (Exception e) {
			logger.error(e);
			return new UserStatus(0, e.toString());
		}
