...
import com.nextech.dreamConstruction.dto.MultipleProduct;
	...
	public @ResponseBody UserStatus addProduct(
		...
		try {
			...
			if (productService.getProductByName(productDTO.getName()) != null) {
				return new UserStatus(0, messageSource.getMessage(DreamConstructionConstants.PRODUCT_NAME_SHOULD_BE_UNIQUE, null, null));
			}
			if (productService.getProductByPartNumber(productDTO.getProductCode()) != null) {
				return new UserStatus(0, messageSource.getMessage(DreamConstructionConstants.PART_NUMBER_SHOULD_BE_UNIQUE, null, null));
			}
